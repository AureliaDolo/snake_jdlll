...
async fn main() {
    let mut snake = Snake {
        head: (0, 0),
        dir: (1, 0),
        body: VecDeque::with_capacity(8),
    };
    let mut fruit: Point = (rand::gen_range(0, SQUARES), rand::gen_range(0, SQUARES));
    let mut blue: Point = (rand::gen_range(0, SQUARES), rand::gen_range(0, SQUARES));
    let mut orange: Point = (rand::gen_range(0, SQUARES), rand::gen_range(0, SQUARES));
    let mut score = 0;
    let mut speed = 0.3;
    ...
    loop {
        if !game_over {
            ...
            if get_time() - last_update > speed {
                ...
                if snake.head == blue {
                    snake.head = orange;
                    blue = (rand::gen_range(0, SQUARES), rand::gen_range(0, SQUARES));
                }
                if snake.head == orange {
                    snake.head = blue;
                    orange = (rand::gen_range(0, SQUARES), rand::gen_range(0, SQUARES));
                }
                if snake.head.0 == -1 {
                ...
                if snake.head.0 == SQUARES {
                    snake.head.0 = 0;
                }
                if snake.head.1 == SQUARES {
                    snake.head.1 = 0;
                }
        ...
        if !game_over {
            ...
            draw_rectangle(
                offset_x + blue.0 as f32 * sq_size,
                offset_y + blue.1 as f32 * sq_size,
                sq_size,
                sq_size,
                BLUE,
            );
            draw_rectangle(
                offset_x + orange.0 as f32 * sq_size,
                offset_y + orange.1 as f32 * sq_size,
                sq_size,
                sq_size,
                ORANGE,
            );
